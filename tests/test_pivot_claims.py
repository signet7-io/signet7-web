...
class AgentActionGatingPivotTests(unittest.TestCase):
    ...
    def test_front_door_owns_consequential_email(self) -> None:
        ...
        self.assertIn("call them on a number you already have", home)
    ...
    def test_homepage_is_a_customer_front_door(self) -> None:
        ...
        self.assertIn("pip install signet7-hold", home)
        ...
        self.assertIn("https://qual.signet7.io/account", home)
        self.assertIn(">Account</a>", home)
...
class ContentSecurityPolicy(unittest.TestCase):
    ...
    def test_every_page_has_theme_switch_and_dark_scene_asset(self) -> None:
        ...
        for name, html in self.pages.items():
            with self.subTest(page=name):
                ...
                if name == "index.html":
                    ...
                    self.assertNotIn("src=\"assets/theme.js", html)
                    continue
        ...
        home = self.pages["index.html"]
        self.assertIn("The last look before money moves.", home)
        self.assertIn("tech-scene.jpg", home)
        self.assertNotIn("tech-scene-dark.jpg", home)
        self.assertIn("data-inspect", home)
        self.assertIn("Two steps.", home)
        self.assertIn('data-panel="inspect"', home)
        self.assertIn('data-panel="received"', home)
        motion = (ROOT / "assets" / "motion.js").read_text(encoding="utf-8")
        self.assertIn("data-inspect", motion)
        self.assertIn("showPanel", motion)
        home = self.pages["index.html"]
        self.assertIn("Check the seal. Keep a record. Then call.", home)
        self.assertIn("trust layer for consequential email", home.lower())
        self.assertIn("VSN check hosted on qual", home)
