...
class ContentSecurityPolicy(unittest.TestCase):
    ...
    def test_download_page_does_not_lead_with_vsn(self) -> None:
        """Recipients never need the word VSN. Download is a customer page, not docs."""
        page = self.pages["download.html"]
    def test_enterprise_page_does_not_lead_with_vsn(self) -> None:
        """Recipients never need the word VSN. /enterprise is a customer page, not docs."""
        page = self.pages["enterprise.html"]
    def test_integrations_page_does_not_lead_with_vsn(self) -> None:
        """Recipients never need the word VSN. /integrations is a customer page, not docs."""
        page = self.pages["integrations.html"]
    def test_scenarios_page_does_not_lead_with_vsn(self) -> None:
        """Recipients never need the word VSN. /scenarios is a customer page, not docs."""
        page = self.pages["scenarios.html"]
    def test_smtp_page_does_not_lead_with_vsn(self) -> None:
    ...
    def test_companies_page_does_not_lead_with_vsn(self) -> None:
        ...
        self.assertIn("A company can prove the email before anyone acts.", page)
        self.assertIn("public lookup lists Signet7", page)
        self.assertNotIn("safe to pay", page.lower())
        self.assertNotIn("Verifiable Sender Network", page)
        self.assertNotIn("Enroll VSN", page)
        ...
        self.assertIn("Look up a company", page)
        self.assertIn("Company listing lookup is", page)
        self.assertIn("Keep your inbox. We sit on the money step.", page)
        self.assertIn("https://verify.signet7.io/vsn", page)
        self.assertIn("What a clerk actually does.", page)
        self.assertIn("https://verify.signet7.io/vsn", page)
        self.assertIn("Stay in Outlook. We stamp the send.", page)
